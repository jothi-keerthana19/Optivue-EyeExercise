...
class EnhancedEyeTracker:
    ...
    def process_and_draw_frame(self, frame: np.ndarray) -> Tuple[Dict[str, Any], np.ndarray]:
        ...
        try:
            ...
            print("Reinitializing MediaPipe model...")

            # Reinitialize the model to recover from error state
            self._initialize_model()

            # Mark frame as writeable again
            rgb_frame.flags.writeable = True

            cv2.putText(annotated_frame, "Face detection recovering...", (50, 50),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 165, 0), 2, cv2.LINE_AA)
            return {'face_detected': False, 'success': True, 'recovering': True}, annotated_frame
    ...
    def process_frame(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            self._initialize_model()
            rgb_frame.flags.writeable = True
            return {'face_detected': False, 'success': True, 'recovering': True}
